...
**Honest partial — live ⌘⌫ keypress NOT eyeballed end-to-end:**
- The forget *logic* is unit-tested end-to-end (store + index + selection clamp) and the key wiring is inspected. But I could **not** confirm the live ⌘⌫ keypress in the running app: the palette is a `.nonactivatingPanel` and Carbon `RegisterEventHotKey` does not respond to System-Events-synthesized keys, so I couldn't reliably summon + focus the panel to inject the chord — and the attempt clashed with the user actively using the Mac (focus races). Seeding 5 junk records + driving ⌃⌘Space/⌥⌘J/menu-click all failed to route keys to the panel; junk stayed in `frecency.json` because the keystrokes hit the frontmost app (Safari/Zed), not Haunts. NOT a code failure — a GUI-driving limitation. The user can confirm with a real ⌘⌫ keypress. Test state (junk folder, junk records, ⌥⌘J rebind) was fully restored afterward.
